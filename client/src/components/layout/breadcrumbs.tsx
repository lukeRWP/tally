  ...
  return (
    <div className="mb-3">
      ...
      <nav className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.04em] text-[var(--color-text-muted)] overflow-x-auto min-h-[44px] -my-2 py-2">
        {/* The links fill the rail's existing 44px on a coarse pointer rather
            than sitting 31px tall inside it — the row is already that tall, so
            nothing moves. (globals.css → Touch targets.) */}
        <Link to="/" className="hover:text-[var(--color-text-secondary)] transition-colors flex items-center gap-1 shrink-0 py-2 min-h-[var(--tap-min)]">

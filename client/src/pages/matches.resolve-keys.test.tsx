...
test('1 picks the first candidate of the open row — the same call "Use this" makes', () => {
...
test('the Nth digit picks the Nth candidate card, in rendered order', () => {
...
test('a digit past the row\'s own candidate count does nothing — no falling through to a neighbouring meaning', () => {
...
test('d dismisses the open row — the same call "None of these" makes', () => {
  renderMatches(['/matches?sel=1']);
...
test('the keys are inert while typing — a digit in a field stays a digit', () => {
  renderMatches(['/matches?sel=1']);
...
test('the keys act on the OPEN row, never the browsing cursor — j to a row does not arm it', () => {
  renderMatches();

  fireEvent.keyDown(window, { key: 'j' });   // cursor on row 1, nothing open
  fireEvent.keyDown(window, { key: '1' });
  fireEvent.keyDown(window, { key: 'd' });
...
test('the keys do nothing on a row that has no decision to make (queued/none)', () => {
...
test('the keys are dead while a resolve is in flight, exactly like the disabled buttons', () => {
  pending = true;
  renderMatches(['/matches?sel=1']);
...
test('a held key does not walk down the backlog — auto-repeat is dropped', () => {
  renderMatches(['/matches?sel=1']);
...
test('the keys are off where there is no keyboard to serve (phone, no split)', () => {

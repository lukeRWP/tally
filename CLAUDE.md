...
- `middleware/error-handler.js` maps MySQL codes: `ER_DUP_ENTRY` → 409, missing FK rows → 400, and `ER_LOCK_DEADLOCK` / `ER_LOCK_WAIT_TIMEOUT` → **409 + `Retry-After`** ("please retry"; the pool already retries single-statement deadlocks once, `withTransaction` never does, so a transactional deadlock reaches the client as a retryable conflict, not a 500 — #354). Redaction covers `password/token/secret/cookie`, `authorization`, `apiKey`/`api_key`/`x-api-key`, `client_secret`, and OAuth `access/refresh/id_token` spellings, in body, params, query and the URL query string.
...
- **Product images on a share page come only from catalogue-provider hosts.** `products.IMAGE_URL` is a shared, first-writer-wins column (there is no product update route), so `server/src/modules/products/image-url.js` gates it twice (#355): `storableImageUrl()` (https only, no credentials, ≤2000 chars — applied in the Joi schema AND in `ProductsService.create()`, because the lookup auto-save never sees the schema) and `publicImageUrl()` (the `PUBLIC_IMAGE_HOSTS` suffix allowlist, applied to every `productImageUrl` the share service emits). A host not on the list renders as no image, never an error — extend the list when a lookup provider starts returning a new CDN. The catalogue stays global by decision: per-property products would need a migration and a re-link of every item, for a column the authenticated app already lets anyone see.
